...
export class RoyalThreeDifference extends Room<GameState> {
  ...
  var2CardUtils: Var2CardUtils; //Utilities for Cards
  ...
  onCreate(options: any) {
    ...
    this.var2CardUtils = new Var2CardUtils();
  ...
  initializeMessageHandlers() {
    ...
    this.onMessage('Standup', (client, message) => {
      ...
      if (this.playerCount == 1 && this.state.isGameRunning) {
        ...
        unpackplayer = this.var2CardUtils.computeHands(
          unpackplayer,
    ...
    //TODO : Changes according to joker card in card utils
    this.onMessage("extraCard", (client, message) => {
      ...
      let newDeck = this.var2CardUtils.pushCard(this.state.deck, extraCard[0]);
    ...
    this.onMessage(`chooseCardFromDeck`, (client, message) => {
      ...
      let newExtraCard = this.var2CardUtils.getCardsFromDeck(
        this.state.deck,
        player
      );

      if (newExtraCard.gotCard) {
        this.state.deck = newExtraCard.deck;
        ...
        this.broadcast("actions", { seat: this.state.players[client.id].Seatnumber.toString(), action: "New Card", bet: this.state.players[client.id].currentBet.toString() });
      }
      else {
    ...
    this.onMessage(`SideShow_request`, (client, message) => {
      if (message) {
        this.sideshowPlayers = this.var2CardUtils.computeHands(
  ...
  //Start game after enough player connected in a room
  startGame() {
    this.clearState();
  ...
  distributeCards(client: Client) {
    this.state.deck = this.var2CardUtils.getDeck();

    for (let i = 0; i < this.pokerConfig.holeCards; i++) {
      for (let playerId in this.state.players) {
        let player: Player = this.state.players[playerId];
        let res = this.var2CardUtils.popCards(this.state.deck, 1);
        this.state.deck = res.deck;

        //rs.chosenCards[0].isHole = true;
  ...
  jokerCard() {
    ...
    let jokerCard = this.var2CardUtils.popCards(this.state.deck, 1);
    this.state.deck = jokerCard.deck; //Update new Deck to State deck
  ...
  chooseBlinds() {
    var selected_index: number = this.playerUtils.getRandomPlayer(
      this.CurrentSeattingArrange.length
    );
  ...
  joker_Calculation(player: Player) {
    let replaced = this.var2CardUtils.checkJokerCards(
      player,
    ...
    let cardHand = this.var2CardUtils.computeHandsForCards(player, 3, player.cards, this.playerUtils.rankByHand);
    let replaceCardHand = this.var2CardUtils.computeHandsForCards(player, 3, player.replacedCards, this.playerUtils.rankByHand);
