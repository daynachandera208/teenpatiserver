...
export class Var2CardUtils {
	...
	getCardsFromDeck(deck: ArraySchema<Card>, player: Player) {
		...
		if (playerChips > cardForChips) {		//Check if player had enough chips
			...
			if (player.newCardCount < 3) {
				if (player.cards.length === 3) {
					...
					player.newCardCount++;
					player.totalChips -= cardForChips;
					gotCard = true;
				}
			}
			else {
